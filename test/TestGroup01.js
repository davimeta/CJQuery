...
//3
{
    STEP_INDEX++;
    var COLOR = 'red';
    $c('li:nth-child(1)').css('color', COLOR);
...
{
    STEP_INDEX++;
    var q = $c('.test').find('p:nth-child(1)');
...
{
    STEP_INDEX++;
    var q = $c('.test > ul > li').findAt(2, 'p:nth-child(1)');
    q.css('color', 'blue');
    isSuccess = (q != null && q.length > 0);
    resultMessage = isSuccess ? "OK(success to query child selectors)" : "Fail";
    AssertTrue(isSuccess, resultMessage);
...
//7
{
    STEP_INDEX++;
    isSuccess = false;
    $c('.test > ul > li > p').each(function(index, p) {
        isSuccess = true;
        if(index % 2 == 1){
